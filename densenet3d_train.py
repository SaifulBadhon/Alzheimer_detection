...
train_loader = DataLoader(FMRIDataset(train_df, root_folder), batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
test_loader = DataLoader(FMRIDataset(test_df, root_folder), batch_size=batch_size, num_workers=4, pin_memory=True)

# ===============
# Define DenseNet3D model
# ===============
model = DenseNet121(
    spatial_dims=3,
    in_channels=1,
...
pd.DataFrame(metrics).to_csv(os.path.join(output_dir, "densenet3d_metrics.csv"), index=False)

# Save classification report and confusion matrix
with open(os.path.join(output_dir, "densenet3d_classification_report.txt"), "w") as f:
    f.write("Classification Report:\n")
    f.write(report + "\n\n")
    f.write("Confusion Matrix:\n")
    f.write(str(cm))

print("✅ Metrics saved to densenet3d_metrics.csv")
print("✅ Classification report and confusion matrix saved to densenet3d_classification_report.txt")
